...
struct ZipEntry: Equatable {
  ...
  var compressedSize: UInt64 {
    if centralDirectoryStructure.isZIP64 {
      ...
      return try! (UInt64(dataDescriptor?.compressedSize ?? centralDirectoryStructure.compressedSize) | 0)
  }
  ...
  var uncompressedSize: UInt64 {
    if centralDirectoryStructure.isZIP64 {
      ...
      return try! (UInt64(dataDescriptor?.uncompressedSize ?? centralDirectoryStructure.uncompressedSize) | 0)
  }
  ...
  var dataOffset: UInt64 {
    ...
    dataOffset += try! (UInt64(localFileHeader.fileNameLength) | 0)
    dataOffset += try! (UInt64(localFileHeader.extraFieldLength) | 0)
    return dataOffset
  }
...
extension ZipEntry.CentralDirectoryStructure {
  var effectiveCompressedSize: UInt64 {
    ...
    return try! (UInt64(compressedSize) | 0)
  }
  ...
  var effectiveUncompressedSize: UInt64 {
    ...
    return try! (UInt64(uncompressedSize) | 0)
  }
  ...
  var effectiveRelativeOffsetOfLocalHeader: UInt64 {
    ...
    return try! (UInt64(relativeOffsetOfLocalHeader) | 0)
  }
}
