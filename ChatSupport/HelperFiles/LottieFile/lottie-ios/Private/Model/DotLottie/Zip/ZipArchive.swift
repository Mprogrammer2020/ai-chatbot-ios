...
final class ZipArchive: Sequence {
  ...
  var totalNumberOfEntriesInCentralDirectory: UInt64 {
      try! zip64EndOfCentralDirectory?.record.totalNumberOfEntriesInCentralDirectory
      ?? UInt64(endOfCentralDirectoryRecord.totalNumberOfEntriesInCentralDirectory)
  }

  var sizeOfCentralDirectory: UInt64 {
    try! zip64EndOfCentralDirectory?.record.sizeOfCentralDirectory
      ?? UInt64(endOfCentralDirectoryRecord.sizeOfCentralDirectory)
  }

  var offsetToStartOfCentralDirectory: UInt64 {
    try! zip64EndOfCentralDirectory?.record.offsetToStartOfCentralDirectory
  ...
  func makeIterator() -> AnyIterator<ZipEntry> {
    ...
    return AnyIterator {
      ...
      if centralDirStruct.usesDataDescriptor {
        let additionalSize = try! UInt64(localFileHeader.fileNameLength) + UInt64(localFileHeader.extraFieldLength)
      ...
      defer {
        ...
        directoryIndex += try! (UInt64(centralDirStruct.fileNameLength) | 0)
        directoryIndex += try! (UInt64(centralDirStruct.extraFieldLength) | 0)
        directoryIndex += try! (UInt64(centralDirStruct.fileCommentLength) | 0)
        index += 1
      }
      return ZipEntry(
